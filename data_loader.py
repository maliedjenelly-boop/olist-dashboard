...
@st.cache_data
def load_all(uploaded_files=None):
    """
    Charge et nettoie toutes les tables Olist.
    Fonctionne en local (CSV dans data/olist/) 
    ou via upload manuel sur Streamlit Cloud.
    """
    files_needed = {
    ...
    # Mode 1 : fichiers uploadés via st.file_uploader
    if uploaded_files:
        uploaded_dict = {f.name: f for f in uploaded_files}
        for key, filename in files_needed.items():
            if filename in uploaded_dict:
                dfs[key] = pd.read_csv(uploaded_dict[filename])
            else:
                st.warning(f"Fichier manquant dans l'upload : {filename}")
                st.stop()

    # Mode 2 : fichiers locaux (local ou dossier data/ sur le repo)
    else:
        for key, filename in files_needed.items():
