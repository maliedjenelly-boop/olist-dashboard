...
if local_data_exists and st.session_state["df"] is None:
    ...
    if uploaded:
        ...
        if missing:
            ...
            with st.spinner("Traitement des données…"):
                st.session_state["df"] = load_all(uploaded_files=uploaded)
            st.success("Données chargées ! Rechargement en cours…")
            st.rerun()
    st.stop()
